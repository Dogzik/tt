...
let rec proof_inner expr solution map_free map_bond ctx cur_tab ind = match expr with
  ...
  | Lambda(Name(s), p) -> begin
                            ...
                            let (p_s, p_t, tail_proof, ind1) = proof_inner p solution map_free map_bond tail_ctx (cur_tab ^ tab) (ind + 1) in
                            ...
                            (cur_s, cur_t, cur_proof, ind1 + 1)
                          end
  | Apl(p, q) ->  begin
                    let new_tab = cur_tab ^ tab in
                    let (p_s, p_t, p_proof, ind1) = proof_inner p solution map_free map_bond ctx new_tab ind in
                    let (q_s, q_t, q_proof, ind2) = proof_inner q solution map_free map_bond ctx new_tab ind1 in
